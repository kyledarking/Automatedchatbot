...
async function convertImageToCaption(imageURL, api, event, inputText) {
  try {
    ...
    const response = await axios.get(`https://haze-gemini-v-8ba147453283.herokuapp.com/text=${encodeURIComponent(inputText)}&image_url=${encodeURIComponent(imageURL)}`);
...
module.exports.handleEvent = async function ({ api, event }) {
  ...
  try {
    const response = await axios.get(`https://haze-gemini-v-8ba147453283.herokuapp.com/text=${encodeURIComponent(inputText)}`);
