	...
	try {
		...
		if (query) {
			...
			const processingMessage = await api.sendMessage(
				`Asking 🤖 𝗸𝘆𝗹𝗲𝗔𝗶. Please wait a moment...`,
				event.threadID
			);
			...
			if (response.data && response.data.message) {
				...
				console.log(`Sent 🤖 𝗸𝘆𝗹𝗲𝗔𝗶 response to the user`);
			} else {
				throw new Error(`Invalid or missing response from 🤖 𝗸𝘆𝗹𝗲 API`);
			}

			await api.unsendMessage(processingMessage.messageID);
		}
	} catch (error) {
		console.error(`❌ | Failed to get 🤖 𝗸𝘆𝗹𝗲𝗔𝗶 sponse: ${error.message}`);
